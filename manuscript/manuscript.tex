...
\title[Selection calibration for partial-Dyson searches]{Selection-function calibration for Gaia--2MASS--WISE partial-Dyson searches: source-level coupling and real-image response}
...
\begin{abstract}
Multi-stage rare-object searches require a selection function matched to the actual candidate operator, particularly when signal injection changes both source photometry and downstream quality or image-domain decisions. We develop an auditable selection-calibration workflow for Gaia DR3--2MASS--AllWISE partial-Dyson searches. A ten-band injection--recovery experiment based on 265 real stellar templates, 3,000 independent validation stars, and a 220,745-model library yields a conditional SED-recognition response of 0.910 across a 54-cell waste-heat grid and remains stable under conservative noise and WISE-response sensitivity tests. Pairing each injection with the baseline Gaia/WISE host state of the same source gives a joint response of 0.414, compared with 0.402 from multiplying stage-averaged efficiencies; a source-cluster bootstrap places the difference at 0.0118 with a 95 per cent interval of 0.0089--0.0147. Real AllWISE leave-one-host-out image injections then separate morphology response from field environment and detector-linearity terms. For 23 baseline-clean unsaturated W3 hosts, retention remains unity across metric and threshold ablations, independent empirical-PSF splits, moderate PSF broadening, and subpixel offsets, corresponding to a one-sided 95 per cent host-level lower bound of 0.878. W4 is strongly PSF-model dependent, with retention falling to 0.451 under mild broadening and 0.362 for a 0.1-pixel offset. The framework identifies which response terms are measured and which remain uncalibrated, providing a reproducible route toward selection-consistent population inference without conflating conditional pipeline responses with end-to-end completeness.
\end{abstract}

\begin{keywords}
Data Methods -- methods: statistical -- catalogues -- surveys -- infrared: stars -- extraterrestrial intelligence
\end{keywords}
...
\label{fig:cmd}
{\footnotesize\noindent\textit{Alt text:} Density plot of absolute Gaia $G$ magnitude versus $G_{\rm BP}-G_{\rm RP}$ colour for the nominal 100-pc sample. The main-sequence locus runs diagonally from bright blue stars at upper left to faint red stars at lower right. Overlaid giant, white-dwarf/high-absolute-magnitude, and $M_G$-range boundaries delimit the retained main-sequence region. The annotation reports 574,531 parent sources, 250,909 retained CMD main-sequence sources, and 204,613 retained after RUWE $<1.4$.\par}
\end{figure}
...
\label{fig:photrec}
{\footnotesize\noindent\textit{Alt text:} Heat map of conditional ten-band SED-recognition response over nine injected waste-heat temperatures from 100 to 700 K and six covering factors from 0.1 to 0.9. Recovery is close to unity over most of the grid. The largest reduction occurs at covering factor 0.9, especially at the coldest temperatures, with recovery increasing toward warmer temperatures.\par}
\end{figure}
...
\label{fig:shapley}
{\footnotesize\noindent\textit{Alt text:} Bar chart of Shapley shares of baseline observed host-selection loss for five cuts. RUWE is the largest contributor at about 0.39 of the total, followed by $G_{\rm var}$ and stellar-classification probability at about 0.21 and 0.20, AllWISE extension flag at about 0.12, and H$\alpha$ at about 0.09.\par}
\end{figure}
...
\end{equation}
The bootstrap interval therefore remains positive under source-level resampling of this validation set. This does \emph{not} convert the baseline host gate into a counterfactual host completeness; it shows that stage-average multiplication does not preserve the observed source-level dependence.
...
{\footnotesize\noindent\textit{Alt text:} Heat map of same-source photometric recovery combined with the baseline observed host gate across the partial-Dyson grid. The response is nearly flat at roughly 0.4--0.45 for covering factors up to 0.7. At covering factor 0.9 it is lower, especially for cold injected temperatures, and rises toward warmer temperatures.\par}
\end{figure}
...
{\footnotesize\noindent\textit{Alt text:} Line plot comparing W3 and W4 conditional injection retention under PSF-mismatch challenges. W3 remains at 1.0 for the matched case, mild and moderate broadening, and 0.1-, 0.25-, and 0.5-pixel offsets. W4 is 1.0 only in the matched case and falls to 0.451 with mild broadening, 0.171 with moderate broadening, 0.362 at a 0.1-pixel offset, 0.136 at 0.25 pixel, and 0.042 at 0.5 pixel.\par}
\end{figure}

The two bands therefore warrant different interpretations. For these 23 clean, unsaturated hosts, co-spatial W3 morphology retention is robust to the tested operator perturbations. The W4 matched-PSF result is an operator-conditional diagnostic, not a robust completeness measurement. Accordingly, only the W3 result is used for a robust host-level morphology statement.

\subsection{Image-stage decomposition}

We write the image-stage response as a conditional chain, which makes the conditioning between stages explicit. Let $E$ denote an acceptable baseline environment, $L$ a detector/catalogue regime in which the adopted linearized injection operator is valid, and $M$ morphology acceptance under a specified image operator $\mathcal{O}_{\rm image}$. Then
\begin{equation}
C_{\rm image}
...
\section{Selection-consistent population inference}
...
\subsection{Selection-function calibration as a pipeline problem}
...
The W3 results support the geometric expectation over the tested range: the 23 clean, unsaturated hosts survive not only the matched-PSF LOHO injection but also threshold/feature ablations, independent empirical-PSF splits, moderate PSF broadening, and subpixel offsets. W4 shows that the W3 result does not generalize across bands. Its matched-PSF experiment also gives 23/23 host successes, but small PSF perturbations cause substantial losses. Thus ``co-spatial point source'' is not by itself sufficient to guarantee a stable image-selection response when the scoring operator is PSF-sensitive.
...
\subsection{Implications for selection-consistent population inference}
...
\subsection{Transferability to other multistage rare-object searches}

The calibration logic developed here is not specific to technosignatures. Any rare-object search that combines catalogue availability, signal-dependent photometry, deterministic source-quality cuts, image-domain screening, and follow-up classification can exhibit the same distinction between conditional stage response and end-to-end completeness. Injection at only one upstream stage does not calibrate downstream decisions, and multiplying stage-average efficiencies is valid only when their source dependence is negligible or explicitly modelled. The practical prescription is therefore to freeze the candidate operator, preserve source identity through injected stages, and report response surfaces together with their conditioning set rather than compressing heterogeneous stages into a single scalar.

This perspective is especially useful when proprietary, learned, or manually applied stages cannot be reproduced exactly. In that case, the missing response should remain an explicit term in the inference rather than being replaced by an unrelated upstream efficiency. Real-data injection, operator perturbation, and source-level resampling can still identify which components are robust, which are model-dependent, and which require new calibration data.
...
\textbf{AI-use disclosure:} OpenAI ChatGPT was used as an AI-assisted tool for literature-search and source-checking support, methodological discussion, code prototyping and debugging, exploratory numerical-analysis scripting, robustness-test design, manuscript drafting, and language editing. All reported numerical results were generated or reproduced through inspectable code and archived outputs. The author reviewed the source data, methods, executable code, numerical results, references, interpretation, and final manuscript and accepts full responsibility for the work.
...
The public survey data used in this work are available from the Gaia Archive and the NASA/IPAC Infrared Science Archive. Analysis scripts underlying the reported results, input hashes, frozen result tables, figure products, and claim-boundary documentation are publicly available at \url{https://github.com/Wujijiandao/gtco-dyson-selection-calibration}. The frozen \texttt{v1.0.0-submission} software and reproduction release underlying the reported numerical results is archived on Zenodo at \url{https://doi.org/10.5281/zenodo.21896997} \citep{Zhang2026GTCO}. The real AllWISE cutouts used for the 24-field image-response experiment can be regenerated from source coordinates and the per-file IRSA product manifest supplied in the repository. Bug reports and reproducibility questions may be submitted through the repository issue tracker.
