...
def score_subset(x,trainX,idxs):
    X=trainX[:,idxs]; xx=x[idxs]
    med=np.nanmedian(X,axis=0); mad=1.4826*np.nanmedian(np.abs(X-med),axis=0); mad=np.where(mad>1e-8,mad,1)
    return float(np.nanmax((xx-med)/mad))

Tgrid=[100,150,200,250,300]; Ggrid=[.03,.05,.08,.10,.14,.16]
sids=list(seed.index)
feature_sets={
    "full_4metric":[0,1,2,3],
    "no_pseudochi":[1,2,3],
    "geometry_only":[2,3],
    "residual_only":[1],
}
...
for band in ("W3","W4"):
    for test in sids:
        ...
        trainXA=[features(s,band,p) for s in train]
        trainX=np.stack([a[0] for a in trainXA])
        x0,A0=features(test,band,p)
        st=seed.loc[test]; mag=float(st.w3mpro if band=="W3" else st.w4mpro)
        Fs=jy(mag,ZP[band]); Fbol=float(st.Fbol_fit_Wm2)
        # Precompute injected feature vectors once.
        inj_cache=[]
        for T in Tgrid:
            ...
            for g in Ggrid:
                ...
                inj_cache.append((T,g,sat,xi))
        for fs_name,idxs in feature_sets.items():
            tr_scores=np.array([score_subset(x,trainX,idxs) for x in trainX])
            s0=score_subset(x0,trainX,idxs)
            for q in quantiles:
                thr=float(np.quantile(tr_scores,q,method="linear")) if q<1 else float(tr_scores.max())
                bpass=s0<=thr+1e-12
                for T,g,sat,xi in inj_cache:
                    si=score_subset(xi,trainX,idxs)
                    rows.append(dict(source_id=test,band=band,feature_set=fs_name,threshold_quantile=q,
                                     T_DS_K=T,gamma=g,baseline_pass=bpass,catalog_saturation_risk=sat,
                                     injected_pass=si<=thr+1e-12,baseline_score=s0,injected_score=si,score_change=si-s0))

df=pd.DataFrame(rows)
out=OUT/"gtco_submission_image_metric_threshold_robustness.csv.gz"
df.to_csv(out,index=False,compression="gzip")
s=[]
for (band,fs,q),d0 in df.groupby(["band","feature_set","threshold_quantile"]):
    d=d0[d0.baseline_pass & ~d0.catalog_saturation_risk]
    if len(d)==0: continue
    byhost=d.groupby("source_id").agg(all_pass=("injected_pass","all"),n=("injected_pass","size"))
    s.append(dict(band=band,feature_set=fs,threshold_quantile=q,
                  N_baseline_clean_hosts=d.source_id.nunique(),N_valid_injections=len(d),
                  injection_retention=d.injected_pass.mean(),hosts_all_pass=int(byhost.all_pass.sum()),
                  hosts_with_any_failure=int((~byhost.all_pass).sum()),median_score_change=d.score_change.median()))
summary=pd.DataFrame(s)
summary.to_csv(OUT/"gtco_submission_image_metric_threshold_robustness_summary.csv",index=False)
