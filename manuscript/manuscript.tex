...
\begin{table}
\centering
\caption{Photometric robustness tests. The hard four-band saturation-onset cut is a deliberately severe stress test because it eliminates the bright end of the template sequence; it is not the preferred reconstruction.}
...
\begin{table}
\centering
\caption{Conditional retention under injection-PSF mismatch. The scoring PSF and morphology calibration remain LOHO. The disjoint-split case is a severe empirical-PSF stress test.}
...
\item W4 does not support the same robust scalar statement. Its matched-PSF LOHO retention is 1.000, but retention falls to 0.451 under mild PSF broadening and 0.362 for a 0.1-pixel injection offset, with stronger mismatch producing still lower values. The W4 result is therefore evidence of PSF-model sensitivity rather than a morphology-completeness bound.
...
The W3 threshold ablation in Table~\ref{tab:ablation} changes which baseline fields are considered clean but does not produce a signal-induced image failure in any retained field. More importantly, the W3 result survives the PSF-mismatch tests in Table~\ref{tab:psfmismatch}. W4 demonstrates why this extra challenge is necessary: metric/threshold ablation alone would have suggested complete retention, whereas modest injection/scoring PSF mismatch produces substantial failures.

The W3 host-level confidence interval is deliberately conservative with respect to the injection grid. A host with 30 successful grid cells still contributes one independent host-level success, not 30 binomial trials. The reported lower bound should be understood as a summary of the 23 tested clean hosts under the stated challenge family, not a population statement about arbitrary WISE environments. If future work expands the image sample to hundreds of representative fields, host-cluster models can provide tighter and more externally valid estimates while allowing PSF uncertainty and environment class to enter explicitly.
